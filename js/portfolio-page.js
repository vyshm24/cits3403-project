// portfolio-page.js
// JavaScript for the Portfolio / My Profile Page
// Connect to backend by replacing renderPage(myUser) with a fetch() call
...
// ── BANNER UPLOAD ──
document.getElementById("banner").style.background = "linear-gradient(135deg, #f9a8d4, #fcd5b0)";
...
document.getElementById("banner-upload").addEventListener("change", e => {
    ...
    reader.onload = ev => {
        document.getElementById("banner").style.background = `url('${ev.target.result}') center/cover no-repeat`;
    };
...
function renderPage(user) {
    ...
    const allRatings = user.itineraries.flatMap(it =>
        Object.entries(it.ratings||{}).flatMap(([star,count])=>Array(count).fill(Number(star)))
    );
    ...
    if(starsEl && avgRating !== "—") {
        const rounded = Math.round(parseFloat(avgRating));
        starsEl.textContent = [1,2,3,4,5].map(i => i<=rounded ? "★" : "☆").join("");
    }
